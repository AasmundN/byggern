...
void MENU_update_state(menu_t* menu, joystick_dir_t dir){
    ...
    {
    case UP:
        menu->current->selected = menu->current->selected ? menu->current->selected-1 : menu->current->num_items-1;
        break;

    case DOWN:
        menu->current->selected = menu->current->selected==menu->current->num_items-1 ? 0 : menu->current->selected+1;
        break;
